...
class AuthPageView(APIView):
    ...
    def post(self, request):
        ...
        if Users.objects.filter(username=request.data['username']).exists():
            if Users.objects.filter(password=request.data['password'], username=request.data['username']).exists():
                user = request.user
                serializer.is_valid()
                token, created = Token.objects.get_or_create(
                    user=Users.objects.filter(username=request.data['username']).values_list('id', flat=True).first())
            ...
            return Response(
                {'username': serializer.data['username'], 'password': serializer.data['password'], 'token': token.key})
